...
You can use Vue to [build standard Web Components](https://vuejs.org/guide/extras/web-components.html) that can be embedded in any HTML page, regardless of how they are rendered. This option allows you to leverage Vue in a completely consumer-agnostic fashion: the resulting web components can be embedded in legacy applications, static HTML, or even applications built with other frameworks.
...
Vue provides core libraries and [comprehensive tooling support](/scaling-up/tooling) with amazing developer experience for building modern SPAs, including:
...
There are two flavors of SSG: 
- Single-page 
- Multi-page. 

Both flavors pre-render the site into static HTML, the difference is that:

| Feature                | Single-page SSG                                                                                           | Multi-page SSG                                                                                                   |
|------------------------|----------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|
| Hydration              | Hydrates the page into an SPA after initial load                                                          | Loads a new HTML page on every navigation                                                                        |
| JS Payload             | More upfront JS payload and hydration cost                                                                | Minimal or no JS if no interaction is needed; some multi-page SSG like [Astro](https://astro.build/) supports partial hydration for interactive islands                 |
| Navigation             | Faster subsequent navigations (partial updates, no full reload)                                           | Each navigation reloads the entire page                                                                          |
| Best Use Case          | Apps with non-trivial interactivity, deep session lengths, or persistent state across navigations         | Mostly static sites; minimal JS, faster initial loads, can use Vue for interactive "islands"                     |
...
- Use Vue's [Custom Renderer API](https://vuejs.org/api/custom-renderer.html) to build custom renderers, like those for [the terminal](https://github.com/vue-terminal/vue-termui)!

## Quiz

<Quiz title="1. Which of the following is NOT a way to use Vue mentioned in the guide?">
  <QuizOption>
    Building desktop apps with Electron or Wails
  </QuizOption>
  <QuizOption>
    Building mobile apps with Ionic Vue
  </QuizOption>
  <QuizOption>
    Building 3D WebGL experiences with TresJS
  </QuizOption>
  <QuizOption correct>
    Building native iOS apps with Swift
  </QuizOption>
</Quiz>

<Quiz title="2. What is the main advantage of using Server-Side Rendering (SSR) with Vue?">
  <QuizOption>
    It removes the need for JavaScript on the client
  </QuizOption>
  <QuizOption correct>
    It improves time-to-content and SEO by sending pre-rendered HTML to the browser
  </QuizOption>
  <QuizOption>
...
<Quiz title="3. Which scenario is best suited for a single-page SSG (Static Site Generation) with Vue?">
  <QuizOption>
    A mostly static site with no interactivity
  </QuizOption>
  <QuizOption correct>
    An app with non-trivial interactivity and persistent state across navigations
  </QuizOption>
  <QuizOption>
    A site that only uses server-side rendering
  </QuizOption>
  <QuizOption>
    A site that does not use any JavaScript
  </QuizOption>
</Quiz>
