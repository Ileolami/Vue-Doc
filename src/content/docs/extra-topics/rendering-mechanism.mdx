...
description: Learn how Vue renders components
...
In this page, you will learn how Vue take a template and turn it into actual DOM nodes and how Vue updates those DOM nodes efficiently.
...
2. **Mount**: The runtime renderer invokes the render functions, walks the returned virtual DOM tree, and creates actual DOM nodes based on it. This step is performed as a [reactive effect](/extra-topics/reactivity-indepth), so it keeps track of all reactive dependencies that were used.
...
![render pipeline](/rendering-mechanism/render-pipeline.CwxnH_lZ.png)
...
In practice, templates are sufficient for most use cases in applications. Render functions are typically only used in reusable components that need to deal with highly dynamic rendering logic. Render function usage is discussed in more detail in [Render Functions & JSX](/extra-topics/render-function).
...
Both patch flags and tree flattening also greatly improve Vue's [SSR Hydration](https://vuejs.org/guide/scaling-up/ssr.html#client-hydration) performance:
