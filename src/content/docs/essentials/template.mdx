...
<Quiz title="Which of these is true?" multiple>
 <QuizOption correct>Vue compiles the templates into highly-optimized JavaScript code</QuizOption>
 <QuizOption correct>Boolean attributes are attributes that can indicate true / false values by their presence on an element</QuizOption>
 <QuizOption>Each binding can contain multiple expressions</QuizOption>
 <QuizOption correct>Directives are special attributes with the `v-` prefix. </QuizOption>
 <QuizOption>Directives cannot take an “argument”. </QuizOption>
</Quiz>