...
## Quiz
<Quiz title="Which of these is true about `<TransitionGroup>`?" multiple>
<QuizOption>By default, it does render a wrapper element</QuizOption>
<QuizOption correct>A built-in component designed for animating the insertion, removal, and order change of elements or components that are rendered in a list</QuizOption>
<QuizOption correct>Elements inside are always required to have a unique `key` attribute</QuizOption>
<QuizOption >Elements inside are doesn't required to have a unique `key` attribute</QuizOption>
</Quiz>