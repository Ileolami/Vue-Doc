...
description: Learn how to use TypeScript with Composition API in Vue.js
sidebar:
...
<List  title="Prerequisites" icon="open-book">
   - Familiarity with [Using Vue with TypeScript](/typescripts/overview). 
</List>
...
When using type-based declaration, you lose the ability to declare default values for the props. This can be resolved by using [Reactive Props Destructure <Badge text="3.5+" size="small" />](/components-in-depth/props#reactive-props-destructure) :
...
:::caution
When using `withDefaults`, wrap default values for arrays and objects in factory functions to prevent accidental modification between component instances. Each instance will receive its own fresh copy of the default value. This precaution is unnecessary when using destructuring defaults.
:::
...
For runtime declaration, you can use the `PropType` utility type:
...
```
The `props` option is more commonly used with the Options API, so you'll find more detailed examples in the guide to [TypeScript with Options API](/typescripts/options-api#typing-component-props). The techniques shown in those examples also apply to runtime declarations using `defineProps()`.
...
Sometimes you may need to specify complex types for a ref's inner value. You can do that by using the `Ref` type:
...
`reactive()` also infers the type from the initial value:
...
To explicitly type a `reactive` property, you can use interfaces:
...
<Dropdown label="Usage before 3.5" client:load>
...
:::tip
For strict type safety, it is necessary to use optional chaining or type guards when accessing `el.value`. This is because the initial ref value is `null` until the component is mounted, and it can also be set to `null` if the referenced element is unmounted by `v-if`.
:::
...
1. Get its type via `typeof`, 
2. Use TypeScript's built-in `InstanceType` utility to extract its instance type:
...
```
In cases where the component referenced is a [generic component](/typescripts/overview#generic-components), for instance `MyGenericModal`:
...
With `@vue/language-tools` 2.1+, static template refs' types can be automatically inferred and the above is only needed in edge cases.

## Quiz
<Quiz title="1. What is the purpose of the `defineComponent()` helper in Vue with TypeScript?">
  <QuizOption>
    It disables type inference
  </QuizOption>
  <QuizOption correct>
    It enables proper type inference for props, data, and methods
  </QuizOption>
  <QuizOption>
    It only works with Vue 2
  </QuizOption>
  <QuizOption>
...
<Quiz title="2. What is the purpose of the `ref()` function in Vue with TypeScript?">
  <QuizOption>
    It is used to define props
  </QuizOption>
  <QuizOption>
    It is used to define computed properties
  </QuizOption>
  <QuizOption>
    It is used to define methods
  </QuizOption>
  <QuizOption correct>
    It is used to define reactive data
  </QuizOption>
</Quiz>

<Quiz title="3. What is the purpose of the `reactive()` function in Vue with TypeScript?">
  <QuizOption>
    It is used to define props
  </QuizOption>
  <QuizOption>
  ...
  <QuizOption correct>
    It is used to define reactive objects
  </QuizOption>
  <QuizOption>
    It is used to define methods
  </QuizOption>
