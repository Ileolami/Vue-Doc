...
import { Aside, Code, Tabs, TabItem, LinkButton, FileTree } from "@astrojs/starlight/components";
...
- During development, it is recommended to rely on a good [IDE setup](https://vuejs.org/guide/typescript/overview.html#ide-support) for instant feedback on type errors.
...
- Vue CLI also provides TypeScript support, but is no longer recommended. 
   ...
   <LinkButton href="/typescripts/overview#note-on-vue-cli-and-ts-loader" variant="secondary">See Note Below</LinkButton>

### IDE Support​
- [Visual Studio Code (VS Code)](https://code.visualstudio.com/) is strongly recommended for its great out-of-the-box support for TypeScript.
...
- If you intend to use TSX with Vue, set `compilerOptions.jsx` to `"preserve"`, and set [`compilerOptions.jsxImportSource`](https://www.typescriptlang.org/tsconfig#jsxImportSource) to `"vue"`.
...
To let TypeScript properly infer types inside component options, we need to define components with [`defineComponent()`](https://vuejs.org/api/general.html#definecomponent):
...
### Usage with TSX​
Vue also supports authoring components with JSX / TSX. Details are covered in the [Render Function & JSX guide](https://vuejs.org/guide/extras/render-function.html#jsx-tsx).
...
- In SFCs: [`<script setup>` with the generic attribute](https://vuejs.org/api/sfc-script-setup.html#generics)
- Render function / JSX components: [`defineComponent()`'s function signature](https://vuejs.org/api/general.html#function-signature)

---

## API-Specific Recipes​
- [TS with Composition API](/typescripts/composition-api)
- [TS with Options API](/typescripts/options-api)


## Quiz

<Quiz title="1. What is a key benefit of using TypeScript with Vue?">
  <QuizOption>
    It makes the app run faster
  </QuizOption>
  <QuizOption correct>
    It helps catch errors at build time and improves code refactoring confidence
  </QuizOption>
  <QuizOption>
    It disables template compilation
  </QuizOption>
  <QuizOption>
...
<Quiz title="2. Which tool is recommended for type checking Vue Single File Components (SFCs) from the command line?">
  <QuizOption>
    tsc
  </QuizOption>
  <QuizOption correct>
...
<Quiz title="3. How do you enable TypeScript in a Vue Single File Component (SFC)?">
  <QuizOption>
    Use `<script>`
  </QuizOption>
  <QuizOption correct>
  ...
  <QuizOption>
    Add type="typescript" to the `<template>` tag
  </QuizOption>
  <QuizOption>
    Use `<typescript>` instead of `<script>`
  </QuizOption>
</Quiz>

<Quiz title="4. Which official VS Code extension is recommended for Vue 3 TypeScript support?">
  <QuizOption>
    Vetur
  </QuizOption>
  <QuizOption correct>
    Vue - Official (previously Volar)
  </QuizOption>
  <QuizOption>
    VueDX
  </QuizOption>
...
<Quiz title="5. What does the `defineComponent()` helper do in Vue with TypeScript?">
  <QuizOption>
