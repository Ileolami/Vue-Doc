...
description: Learn how to use TypeScript with Options API in Vue.js
sidebar:
...
To annotate complex props types, you can use the `PropType` utility type:
...
You can declare the expected payload type for an emitted event using the object syntax of the `emits` option. Also, all non-declared emitted events will throw a type error when called:
...
You can put `this` type augmentation in a `.ts` file, or in a project-wide `*.d.ts` file. Either way, make sure it is included in `tsconfig.json`. For library / plugin authors, this file should be specified in the `types` property in `package.json`.
...
Without proper type augmentation, the arguments of this hook will implicitly have `any` type. You can augment the `ComponentCustomOptions` interface to support these custom options:
...
The placement of this augmentation is subject to the [same restrictions](/typescripts/options-api#type-augmentation-placement) as global property augmentations.
...
## Quiz
<Quiz title="1. How do you enable type inference for props in the Options API with TypeScript?">
  <QuizOption>
    By using the props option without any helper
  </QuizOption>
  <QuizOption correct>
    By wrapping the component with `defineComponent()`
  </QuizOption>
  <QuizOption>
    By using the `setup()` function only
  </QuizOption>
  <QuizOption>
    By adding types to the template section
  </QuizOption>
</Quiz>

<Quiz title="2. What is the recommended way to annotate complex prop types in the Options API?">
  <QuizOption>
    Use type: Object directly
  </QuizOption>
  <QuizOption correct>
    Use the PropType utility type
  </QuizOption>
  <QuizOption>
    Use type: Array
  </QuizOption>
...
<Quiz title="3. Why is it important to explicitly annotate event handler arguments in TypeScript?">
  <QuizOption>
    To make the code run faster
  </QuizOption>
  <QuizOption correct>
    To avoid implicit any errors and ensure correct type checking
  </QuizOption>
  <QuizOption>
    To enable hot module replacement
  </QuizOption>
  <QuizOption>
    To allow using JavaScript instead of TypeScript
  </QuizOption>
</Quiz>