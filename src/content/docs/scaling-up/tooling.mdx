...
<List title="Try It Online" icon="seti:vue">
  You don't need to install anything on your machine to try out Vue SFCs - there
  are online playgrounds that allow you to do so right in the browser: - <LinkButton
    href="https://play.vuejs.org/"
    icon="seti:video"
    iconPlacement="start"
    variant="secondary">Vue SFC Playground</LinkButton>
  - Always deployed from latest commit - Designed for inspecting component compilation
  results - [Vue + Vite on StackBlitz](https://vite.new/vue) - IDE-like environment
  running actual Vite dev server in the browser - Closest to local setup - It is
  also recommended to use these online playgrounds to provide reproductions when
  reporting bugs. It is also recommended to use these online playgrounds to provide
...
## TypeScript​
Main article: [Using Vue with TypeScript](https://vuejs.org/guide/typescript/overview.html).
...
## Testing​
Main article: [Testing Guide](https://vuejs.org/guide/scaling-up/testing.html).
