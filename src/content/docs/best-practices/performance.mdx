...
<List>
In this section, You will learn about the things you should pay attention to when it comes to performance in a Vue application.
</List>
...
<Card>
_Check out the [SSR Guide](https://vuejs.org/guide/scaling-up/ssr.html) to learn about performing SSR with Vue. If your app doesn't have rich interactivity requirements, you can also use a traditional backend server to render the HTML and enhance it with Vue on the client._
</Card>
...
Instead, you can optimize this by manually comparing the new value with the old value, and conditionally returning the old value if you know nothing has changed:
...
<Aside type="caution">The following tips affect both page load and update performance.</Aside>
...
However, you don't necessarily have to render all these nodes upfront. In most cases, the user's screen size can display only a small subset of your large list. You can greatly improve the performance with list virtualization, the technique of only rendering the items that are currently in or close to the viewport in a large list.
...
Sometimes you may create [renderless components](https://vuejs.org/guide/components/slots.html#renderless-components) or higher-order components (i.e. _components that render other components with extra props_) for better abstraction or code organization. While there is nothing wrong with this, do keep in mind that component instances are much more expensive than plain DOM nodes, and creating too many of them due to abstraction patterns will incur performance costs.

You must take note that reducing only a few instances won't have noticeable effect, so don't sweat it if the component is rendered only a few times in the app. The best scenario to consider this optimization is again in large lists. Imagine a list of 100 items where each item component contains many child components. Removing one unnecessary component abstraction here could result in a reduction of hundreds of component instances.

## Quiz

<Quiz title="1. What is the best way to optimize performance in Vue?">
  <QuizOption>
    Use the production build of Vue
  </QuizOption>
  <QuizOption>
    Use the development build of Vue
  </QuizOption>
  <QuizOption>
    Use the global build of Vue
  </QuizOption>
  <QuizOption correct>
    Use the right architecture for the type of application you are building
  </QuizOption>
</Quiz>

<Quiz title="2. Which aspect does 'page load performance' in Vue primarily focus on?">
  <QuizOption>
    How fast the application updates after user input
  </QuizOption>
  <QuizOption correct>
    How quickly the application shows content and becomes interactive on the initial visit
  </QuizOption>
  <QuizOption>
    How many dependencies are installed
  </QuizOption>
  <QuizOption>
    How often the app is deployed
  </QuizOption>
</Quiz>

<Quiz title="3. What is the main benefit of using code splitting in a Vue application?">
  <QuizOption>
    It increases the size of the JavaScript bundle
  </QuizOption>
  <QuizOption correct>
    It allows loading only the necessary code on demand, improving performance
  </QuizOption>
  <QuizOption>
    It disables lazy loading
  </QuizOption>
  <QuizOption>
    It makes all code load at once
  </QuizOption>
</Quiz>

<Quiz title="4. When dealing with large lists in Vue, what technique can greatly improve rendering performance?">
  <QuizOption>
    Using more component abstractions
  </QuizOption>
  <QuizOption>
    Avoiding the use of computed properties
  </QuizOption>
  <QuizOption correct>
    List virtualization (only rendering visible items)
  </QuizOption>
  <QuizOption>
    Using shallowRef for all data
  </QuizOption>
</Quiz>
