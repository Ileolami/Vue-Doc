...
description: Learn how to use the v-model directive in Vue.js
---

import { FileTree, Icon, Badge, Aside, LinkButton } from '@astrojs/starlight/components';
...
Starting in Vue 3.4, the recommended approach to achieve this is using the [`defineModel()`](https://vuejs.org/api/sfc-script-setup.html#definemodel) macro:
...
<Dropdown label="Pre 3.4 Usage" client:load>
      ```vue
        <!-- MyComponent.vue -->
        <script setup>
        defineProps({
...
When we were learning about form input bindings, we saw that `v-model` h`as built-in modifiers - `.trim`, `.number` and `.lazy`. In some cases, you might also want the `v-model` on your custom input component to support custom modifiers.
...
## Quiz
<Quiz title="1. Which of the following is true about v-model in Vue?" multiple>
  <QuizOption correct>v-model is used to create two-way data bindings</QuizOption>
  <QuizOption>v-model is used to pass data from a parent component to a child component</QuizOption>
  <QuizOption correct>v-model is a directive</QuizOption>
  <QuizOption correct>v-model is used to pass data from a child component to a parent component</QuizOption>
</Quiz>

<Quiz title="2. Which of the following is true about v-model modifiers in Vue?" multiple>
  <QuizOption correct>v-model modifiers can be used to implement custom behavior for the v-model binding</QuizOption>
  <QuizOption>v-model modifiers can be used to pass data from a parent component to a child component</QuizOption>  
  <QuizOption correct>v-model modifiers can be used to modify the value of the v-model binding</QuizOption>
  <QuizOption>v-model modifiers can be used to pass data from a child component to a parent component</QuizOption>
</Quiz>
