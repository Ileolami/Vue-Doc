---
title: Props
description: Learn how to use props in Vue.js
---
...
Vue components require explicit props declaration so that Vue knows what external props passed to the component should be treated as fallthrough attributes (which will be discussed in its [dedicated section](/components/fallthrough)).
...
This not only documents your component, but will also warn other developers using your component in the browser console if they pass the wrong type. You will find more details about [prop validation](/components/props#prop-validation) further down this page.
...
<div style="display:flex;"><p>More details: <a href="typescripts/composition-api#typing-component-props">**Typing Component Props**</a></p> <Icon name="seti:tsconfig" color="blue" size="1.2rem" class="icons" /></div>

---

## Reactive Props Destructure <Badge text="3.5+" size="large" />
...
We use [PascalCase for component tags](/components/registration#component-name-casing) when possible because it improves template readability by differentiating Vue components from native elements. However, there isn't as much practical benefit in using camelCase when passing props, so we choose to follow each language's conventions.
...
If you want to pass all the properties of an object as props, you can use [`v-bind` without an argument](/essentials/template#dynamically-binding-multiple-attributes) (v-bind instead of :prop-name). For example, given a post object:
...
The main drawback of such mutations is that it allows the child component to affect parent state in a way that isn't obvious to the parent component, potentially making it more difficult to reason about the data flow in the future. As a best practice, you should avoid such mutations unless the parent and child are tightly coupled by design. In most cases, the child should [emit an event](/components/events) to let the parent perform the mutation.
...
If using [Type-based props declarations](https://vuejs.org/api/sfc-script-setup.html#type-only-props-emit-declarations) Vue will try its best to compile the type annotations into equivalent runtime prop declarations. For example, `defineProps<{ msg: string }>` will be compiled into `{ msg: { type: String, required: true }}`.
...
## Quiz
<Quiz title="1. Which of the following is true about props in Vue?" multiple>
  <QuizOption correct>Props are used to pass data from a parent component to a child component</QuizOption>
  <QuizOption correct>Props can be of any type</QuizOption>
  <QuizOption>Props can be mutated in the child component</QuizOption>
  <QuizOption correct>Props are read-only in the child component</QuizOption>
</Quiz>

<Quiz title="2. Which of the following is true about prop validation in Vue?" multiple>
  <QuizOption correct>Prop validation is used to specify requirements for props</QuizOption>
  <QuizOption>Prop validation can be used to specify the default value of a prop</QuizOption>
  <QuizOption correct>Prop validation can be used to specify the type of a prop</QuizOption>
  <QuizOption correct>Prop validation can be used to specify a custom validator function</QuizOption>
</Quiz>
