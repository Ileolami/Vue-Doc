...
## Quiz
<Quiz title="1. What is a fallthrough attribute in Vue?">
  <QuizOption>It is an attribute that is passed to a component, but is not explicitly declared in the receiving component's template</QuizOption>
  <QuizOption correct>It is an attribute that is passed to a component, but is not explicitly declared in the receiving component's props or emits</QuizOption>
  <QuizOption>It is an attribute that is passed to a component, but is not explicitly declared in the receiving component's data</QuizOption>
  <QuizOption>It is an attribute that is passed to a component, but is not explicitly declared in the receiving component's methods</QuizOption>
</Quiz>

<Quiz title="2. Which of the following is true about fallthrough attributes in Vue?" multiple>
  <QuizOption correct>They are not automatically added to the root element's attributes if the component renders multiple root elements</QuizOption>  
  <QuizOption>They are automatically added to all elements' attributes if the component renders multiple root elements</QuizOption>
  <QuizOption>They are not automatically added to any element's attributes if the component renders multiple root elements</QuizOption>
  <QuizOption correct>They are automatically added to the root element's attributes if the component renders a single root element</QuizOption>
</Quiz>
