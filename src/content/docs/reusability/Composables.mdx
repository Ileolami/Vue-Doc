---
title: Composables
description: Learn how to use composables in Vue.js
---

import { Aside, Badge, Code, Tabs, TabItem, LinkButton} from "@astrojs/starlight/components";
import { Quiz, QuizOption, List } from 'starlight-videos/components'
import MousePosition from '../../../components/MousePosition.vue';
import Loading from "../../../components/composable/loading.vue"


<List title="Prerequisites" icon="open-book">
  - Familiarity with the Composition API.
  - Familiarity with the [Reactivity Fundamentals](/essentials/reactivity) and [Lifecycle Hooks](https://vuejs.org/guide/essentials/lifecycle.html) chapters.  
</List>
---

## What is a "Composable"?​
In the context of Vue applications, a **composable** is a function that leverages Vue's Composition API to compress and reuse **stateful logic**.

When building frontend applications, logic are mostly resued for common tasks. For example, when you need to format dates in many places, you can create a reusable function for that. This formatter function compress **stateless logic**- it takes some input and immediately returns expected output. There are many libraries out there for reusing stateless logic - for example, [lodash](https://lodash.com/) and [date-fns](https://date-fns.org/), which you may have heard of.

By contrast, stateful logic involves managing state that changes over time. A simple example would be tracking the current position of the mouse on a page. In real-world scenarios, it could also be more complex logic such as touch gestures or connection status to a database.

<Aside type="tip" title="How To Switch To Composition API">
  So far you have being using Options API, To switch to Composition API,
  - Click on the "API Preference" toggle at the top of the left sidebar.
    <img src="/option-API-btn.png" alt="API Preference toggle" width="400" style="display:inline; vertical-align:middle;" />
</Aside>
...
If you are to implement the mouse tracking functionality using the Composition API directly inside a component, it would look like this:
...
But what if you want to reuse the same logic in multiple components? you can extract the logic into an external file, as a composable function:

<Tabs>
<TabItem label="mouse.js">
  <Code
  lang="js"
  code={`
      // mouse.js
...
```
<Aside type="tip">
Each component instance calling `useMouse()` will create its own copies of `x` and `y` state so they won't interfere with one another. If you want to manage shared state between components, read the [State Management](/scaling-up/state-management) chapter.
</Aside>

---

## Async State Example​
The `useMouse()` composable doesn't take any arguments, so let's take a look at another example that makes use of one. When doing async data fetching, you often need to handle different states: `loading`, `success`, and `error`:
...
<Tabs>
  ...
  <TabItem label="Preview"><div class="preview" style="color:white"><Loading client:load/></div></TabItem>
</Tabs>
...
you can refactor our existing implementation with the [`watchEffect()`](https://vuejs.org/api/reactivity-core.html#watcheffect) and [`toValue()`](https://vuejs.org/api/reactivity-utilities.html#tovalue) APIs:
...
The [useFetch() implementation discussed earlier](/reusability/composables#accepting-reactive-state) provides a concrete example of a composable that accepts refs, getters and plain values as input argument.
...
- If you are working on an application that uses [Server-Side Rendering (SSR)](/scaling-up/ssr), make sure to perform DOM-specific side effects in post-mount lifecycle hooks, e.g. `onMounted()`. These hooks are only called in the browser, so you can be sure that code inside them has access to the DOM.
...
### vs. Mixins​
Users coming from Vue 2 may be familiar with the [mixins](https://vuejs.org/api/options-composition.html#mixins) option, which also allows us to extract component logic into reusable units. There are three primary drawbacks to mixins:
...
In the component slots chapter, you will learn about the [Renderless Component](https://vuejs.org/guide/components/slots.html#renderless-components) pattern based on scoped slots. We even implemented the same mouse tracking demo using renderless components.
...
### vs. React Hooks​
If you have experience with React, you may notice that this looks very similar to custom React hooks. Composition API was in part inspired by React hooks, and Vue composables are indeed similar to React hooks in terms of logic composition capabilities. However, Vue composables are based on Vue's fine-grained reactivity system, which is fundamentally different from React hooks' execution model. This is discussed in more detail in the [Composition API FAQ](/extra-topics/composition-api-faq#comparison-with-react-hooks).

### Further Reading​
- [Reactivity In Depth](/extra-topics/reactivity-indepth): for a low-level understanding of how Vue's reactivity system works.
- [State Management](/scaling-up/state-management): for patterns of managing state shared by multiple components.
- [Testing Composables](/scaling-up/testing#testing-composables): tips on unit testing composables.
- [VueUse](https://vueuse.org/): an ever-growing collection of Vue composables. The source code is also a great learning resource.

## Quiz
<Quiz title="1. What is a composable in Vue?">
  <QuizOption>A function that leverages Vue's Options API to compress and reuse stateful logic</QuizOption>
  <QuizOption>A function that leverages Vue's template syntax to compress and reuse stateful logic</QuizOption>
  <QuizOption correct>A function that leverages Vue's Composition API to compress and reuse stateful logic</QuizOption>
  <QuizOption>A function that leverages Vue's reactivity system to compress and reuse stateful logic</QuizOption>
</Quiz>

<Quiz title="2. What is the recommended convention for naming composables in Vue?">
  <QuizOption>To name composable functions with PascalCase names that start with 'use'</QuizOption>
  <QuizOption>To name composable functions with kebab-case names that start with 'use'</QuizOption>
  <QuizOption correct>To name composable functions with camelCase names that start with 'use'</QuizOption>
  <QuizOption>To name composable functions with snake_case names that start with 'use'</QuizOption>
</Quiz>
