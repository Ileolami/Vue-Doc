...
## Quiz
<Quiz title="1. What are the two ways to define a Vue plugin?" multiple>
  <QuizOption correct>As an object with an `install()` method</QuizOption>
  <QuizOption correct>As a function that acts as the install function itself</QuizOption>
  <QuizOption>As a class with a `setup()` method</QuizOption>
  <QuizOption>As a Vue component with special properties</QuizOption>
</Quiz>

<Quiz title="2. How can plugins make functionality available throughout an application?" multiple>
  <QuizOption correct>By registering global components with `app.component()`</QuizOption>
  <QuizOption correct>By adding global properties to `app.config.globalProperties`</QuizOption>
  <QuizOption correct>By making resources injectable with `app.provide()`</QuizOption>
  <QuizOption>By modifying the Vue prototype directly</QuizOption>
</Quiz>
